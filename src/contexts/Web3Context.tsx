...
export const Web3Provider: React.FC<Web3ProviderProps> = ({ children }) => {
  ...
  const getSessions = async (): Promise<Session[]> => {
    ...
    try {
      ...
      let sessionCount = 0;
      try {
        const nextSessionIdResult = await contract.nextSessionId();
        sessionCount = Number(nextSessionIdResult);
      } catch (error) {
        console.error('Contract not deployed or no sessions yet:', error);
        return [];
      }
      ...
      for (let i = 0; i < sessionCount; i++) {
        try {
          ...
          console.error(`Error getting session ${i}:`, error);
          continue;
        }
      }
